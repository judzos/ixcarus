...
// Overlay headers stay outside the explorer's scroll list and search's input
// wrapper (the latter positions tag autocomplete relative to its first child).
const overlayHeader = (label, closeLabel, onClose) => {
  ...
  close.type = "button"
  close.className = "mobile-overlay-close"
  close.setAttribute("aria-label", closeLabel)
  close.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg>'
...
const setupMobileNav = () => {
  ...
  if (drawer) {
    drawer.prepend(overlayHeader(sectionsBtn?.textContent.trim() || "sections", "Close sections", () => {
      shutDrawer()
      ;(iosDock ? dockBtn : sectionsBtn)?.focus({ preventScroll: true })
    }))
  }
  const searchContainer = document.querySelector(".search > .search-container")
  if (searchContainer) {
    searchContainer.prepend(overlayHeader("search the wiki", "Close search", () => {
      // Use the existing backdrop dismissal: it resets the input, autocomplete,
      // results and sidebar stacking together. Do not reproduce search state.
      searchContainer.dispatchEvent(new MouseEvent("click", { bubbles: true }))
      ;(iosDock ? dockBtn : nav.querySelector('[data-target="search"]'))?.focus({ preventScroll: true })
    }))
  }

  // Upstream binds Search only after indexing the corpus. A cold-load tap can
  // precede that binding, even after networkidle. Its autocomplete node is
  // inserted during setup; the observer runs after that synchronous setup has
  // also attached the handlers. Keep the first tap rather than losing it.
  let pendingSearch = null
  const searchBtn = nav.querySelector('[data-target="search"]')
  const cancelSearch = () => {
    pendingSearch?.disconnect()
    pendingSearch = null
    searchBtn?.removeAttribute("aria-busy")
  }
  const requestSearch = () => {
    const open = () => document.querySelector(".search > .search-button")?.click()
    if (!searchContainer || searchContainer.querySelector(".ghost-text")) return open()
    searchBtn?.setAttribute("aria-busy", "true")
    pendingSearch = new MutationObserver(() => {
      if (!searchContainer.querySelector(".ghost-text")) return
      cancelSearch()
      open()
    })
    pendingSearch.observe(searchContainer, { childList: true, subtree: true })
  }

  // CSS lays the home directory out as cards on phones. Explicit native roles
  // preserve table/header relationships in WebKit when rows become grids.
  document.querySelectorAll('body[data-slug="index"] article #sections + .table-container table').forEach((table) => {
    const roles = { table: "table", thead: "rowgroup", tbody: "rowgroup", tr: "row", th: "columnheader", td: "cell" }
    ;[table, ...table.querySelectorAll("thead, tbody, tr, th, td")].forEach((el) => {
      el.setAttribute("role", roles[el.localName])
    })
  })
