(function learningModeRuntime() {
  ...
  const DAY = 86400000
  const MIN = 60000
  // Id on the first field heading in /learn's path grid, and the target of the
  // today card's "Browse paths". Named once because the two are only correct
  // together — an anchor pointing at nothing scrolls nowhere and says nothing.
  const PATHS_ANCHOR = "lm-paths"
  ...
  function load() {
    try {
      ...
      const enrolledMeta =
        s?.enrolledMeta && typeof s.enrolledMeta === "object" ? { ...s.enrolledMeta } : {}
      for (const slug of s?.enrolled ?? []) {
        if (!enrolledMeta[slug]) enrolledMeta[slug] = { on: true, updatedAt: 0 }
      }
      return {
        enrolled: Object.entries(enrolledMeta)
        ...
        settingsUpdatedAt: Number(s?.settingsUpdatedAt ?? 0),
      }
    } catch {
      return {
        enrolled: [],
  ...
  function save(state) {
    try {
      ...
      window.dispatchEvent(new CustomEvent("symposium:state-changed", { detail: { key: KEY } }))
    } catch {
      /* private browsing, quota, or a locked-down profile — nothing to do */
    }
  }

  function setEnrolled(state, slug, on, at = Date.now()) {
    state.enrolledMeta =
      state.enrolledMeta && typeof state.enrolledMeta === "object" ? state.enrolledMeta : {}
    state.enrolledMeta[slug] = { on: !!on, updatedAt: at }
    state.enrolled = Object.entries(state.enrolledMeta)
      .filter(([, entry]) => entry?.on)
      .map(([name]) => name)
      .sort()
  }

  function setTrack(state, id, status) {
    state.tracks = state.tracks && typeof state.tracks === "object" ? state.tracks : {}
    const previous = state.tracks[id] ?? {}
    const now = Date.now()
    state.tracks[id] = {
      ...previous,
      status,
  ...
  function mergeLearning(state, incoming) {
    const incomingMeta =
      incoming?.enrolledMeta && typeof incoming.enrolledMeta === "object"
        ? incoming.enrolledMeta
        : Object.fromEntries(
            (incoming?.enrolled ?? []).map((slug) => [slug, { on: true, updatedAt: 0 }]),
          )
    for (const [slug, entry] of Object.entries(incomingMeta)) {
      const mine = state.enrolledMeta?.[slug]
      if (!mine || Number(entry?.updatedAt ?? 0) > Number(mine?.updatedAt ?? 0)) {
        state.enrolledMeta = state.enrolledMeta ?? {}
        state.enrolledMeta[slug] = entry
      }
    }
    state.enrolled = Object.entries(state.enrolledMeta ?? {})
      .filter(([, entry]) => entry?.on)
      .map(([slug]) => slug)
      .sort()
    for (const [id, entry] of Object.entries(incoming?.tracks ?? {})) {
      const mine = state.tracks?.[id]
      if (!mine || Number(entry?.updatedAt ?? 0) > Number(mine?.updatedAt ?? 0)) {
        state.tracks = state.tracks ?? {}
        state.tracks[id] = entry
      }
    }
    ...
    if (incoming?.daily) {
      const mine = state.daily
      if (!mine || String(incoming.daily.date ?? "") > String(mine.date ?? "")) {
        state.daily = incoming.daily
      } else if (String(incoming.daily.date ?? "") === String(mine.date ?? "")) {
        state.daily = {
          date: String(mine.date ?? ""),
          introduced: Math.max(
            Number(mine.introduced ?? 0),
            Number(incoming.daily.introduced ?? 0),
          ),
          reviewed: Math.max(Number(mine.reviewed ?? 0), Number(incoming.daily.reviewed ?? 0)),
        }
      }
    }
    const mineSettingsAt = Number(state.settingsUpdatedAt ?? 0)
    const incomingSettingsAt = Number(incoming?.settingsUpdatedAt ?? 0)
    if (incoming?.settings && (!state.settings || incomingSettingsAt > mineSettingsAt)) {
      state.settings = { ...DEFAULTS, ...incoming.settings }
    }
    state.settingsUpdatedAt = Math.max(mineSettingsAt, incomingSettingsAt)
    return state
  }
  ...
  function loadProgress() {
    try {
      ...
      return value && typeof value === "object"
        ? value
        : { version: 1, pages: {}, trail: [], seeded: false }
    } catch {
  ...
  function mergeProgress(local, incoming) {
    const out = {
      ...
      pages: { ...(local?.pages ?? {}) },
      trail: [],
      seeded: !!(local?.seeded || incoming?.seeded),
    }
  ...
  function cards() {
    if (!indexPromise) {
      // The schema query is also a one-time escape hatch from the original PWA
      // worker, which incorrectly cached every /static/ URL forever. Keep it in
      // step with buildIndex().version so a schema change cannot reuse an
      // incompatible cached index even before a replacement worker activates.
      indexPromise = fetch(base() + "/static/cards.json?schema=3", { cache: "no-store" })
    ...
    for (const { card, block } of entries) {
      ...
      const paint = () => {
        ...
        const mark = (g) => {
          ...
          setEnrolled(state, card.slug, true)
          const d = daily(state)
  ...
  function wireHard(state, entries, article) {
    ...
    const settled = new Set(
      entries.filter((e) => !isDue(state.sched[e.card.id])).map((e) => e.card.id),
    )
    ...
    const paintBar = (e) => {
      ...
      if (answered(e)) {
        ...
        e.bar.appendChild(el("span", { text: e.card.id in visit ? GRADES[g].label : "answered" }))
      ...
      const mark = (g) => {
        ...
        setEnrolled(state, e.card.slug, true)
        const d = daily(state)
        if (fresh) d.introduced += 1
        d.reviewed += 1
        save(state)
  ...
  function enrolControl(state, index, count) {
    ...
    button.addEventListener("click", () => {
      const on = isOn()
      const now = Date.now()
      for (const slug of scope.slugs) setEnrolled(state, slug, !on, now)
      save(state)
      paint()
    })
    paint()
  ...
  function toolbarHome() {
    ...
    if (slot?.parentElement)
      return { parent: slot.parentElement, style: slot.getAttribute("style") }
  ...
  function toolbar(state, index, onChange) {
    ...
    pop.append(
      ...
      el(
        "div",
        { class: "lm-row" },
        el("label", { for: "lm-on", text: "Cards and scheduling" }),
        onBox,
      ),
      el("p", {
        class: "lm-hint",
      ...
      el(
        "div",
        { class: "lm-row lm-needs-on" },
        el("label", { for: "lm-hard", text: "Hard mode" }),
        hard,
      ),
      el("p", {
        class: "lm-hint",
        text: "Hold the rest of a page back until its cards are answered.",
      }),
      ...thr.rows,
      el(
        "p",
        { class: "lm-foot" },
        el("span", {
          text: here ? here + (here === 1 ? " card here" : " cards here") : "no cards here",
        }),
        el(
          "span",
          {},
          el("a", { href: base() + "/learn", text: "learn" }),
          document.createTextNode(" · "),
          el("a", { href: base() + "/review", text: "review →" }),
        ),
      ),
    )
   ...
   */
  function session(state, pool, eligibleNewSlugs = null) {
    const d = daily(state)
    const sched = state.sched
    const inState = (names) =>
      pool.filter((c) => sched[c.id] && names.includes(sched[c.id].state) && isDue(sched[c.id]))

    const learning = inState(["learning", "relearning"])
    const reviews = inState(["review"])
    const unseenAll = pool.filter((c) => !sched[c.id])
    const unseen = eligibleNewSlugs
      ? unseenAll.filter((c) => eligibleNewSlugs.has(c.slug))
      : unseenAll
    ...
    const reviewRoom =
      state.settings.maxPerDay > 0
        ? Math.max(0, state.settings.maxPerDay - d.reviewed)
        : reviews.length
    ...
    return {
      ...
      unread: Math.max(0, unseenAll.length - unseen.length),
      soon: pending.length,
  ...
  function readableField(field) {
    const names = {
      ...
      "psychology-neuroscience-self-mastery": "Psychology, Neuroscience & Self-Mastery",
    }
    return (
      names[field] ??
      String(field)
  ...
  function readSet(progress) {
    return new Set(
      Object.entries(progress?.pages ?? {})
        .filter(([, entry]) => entry?.tier === "read")
        .map(([slug]) => slug),
    )
  }

  function nextInTrack(track, readPages) {
    return (track.pages ?? []).find((slug) => !readPages.has(slug)) ?? null
  }

  function pageTitles(index) {
    const titles = new Map()
    if (index?.titles && typeof index.titles === "object") {
      for (const [slug, title] of Object.entries(index.titles)) titles.set(slug, title)
    }
    for (const card of index?.cards ?? []) titles.set(card.slug, card.title)
    return titles
  }

  function syllabusList(track, readPages, titles) {
    const next = nextInTrack(track, readPages)
    const pageLesson = (slug) => {
      const read = readPages.has(slug)
      const current = !read && slug === next
      const stateClass = read ? "lm-lesson-read" : current ? "lm-lesson-next" : "lm-lesson-upcoming"
      const stateLabel = read ? "Read" : current ? "Next" : "Upcoming"
      return el(
        "li",
        { class: stateClass, "data-lesson-state": read ? "read" : current ? "next" : "upcoming" },
        el(
          "div",
          { class: "lm-lesson-row" },
          el("a", {
            href: base() + "/" + slug,
            text: titles.get(slug) ?? slug.split("/").pop(),
          }),
          el("span", { class: "lm-lesson-state", text: stateLabel }),
        ),
      )
    }

    if (Array.isArray(track.phases)) {
      const curriculum = el("div", { class: "lm-phases" })
      for (const phase of track.phases) {
        const titleProps = { class: "lm-phase-title", text: phase.title }
        if (phase.description) titleProps.title = phase.description
        const list = el("ol", { class: "lm-syllabus-list" })
        for (const step of phase.steps ?? []) {
          if (step?.page) {
            list.appendChild(pageLesson(step.page))
          } else if (step?.guide) {
            list.appendChild(
              el(
                "li",
                { class: "lm-lesson-guide" },
                el(
                  "div",
                  { class: "lm-lesson-row" },
                  el("a", {
                  ...
                  el("span", { class: "lm-lesson-state", text: "Guide" }),
                ),
              ),
            )
            ...
            list.appendChild(
              el(
                ...
                el(
                  ...
                  el("span", { class: "lm-lesson-state", text: "Practice" }),
                ),
              ),
            )
          }
        }
        curriculum.appendChild(el("div", { class: "lm-phase" }, el("div", titleProps), list))
      }
      return curriculum
    }

    const list = el("ol", { class: "lm-syllabus-list" })
    for (const slug of track.pages ?? []) list.appendChild(pageLesson(slug))
    return list
  }

  function syllabusCard(track, readPages, titles) {
    const done = (track.pages ?? []).filter((slug) => readPages.has(slug)).length
    const total = track.pages?.length ?? 0
    const next = nextInTrack(track, readPages)
    const guides = Array.isArray(track.phases)
      ? track.phases.flatMap((phase) => phase.steps ?? []).filter((step) => step?.guide).length
      : 0
    const meta =
      track.level +
      " · " +
      ...
      total +
      " lessons read" +
      (guides ? " · " + guides + (guides === 1 ? " guide" : " guides") : "")
    return el(
      "section",
      { class: "lm-syllabus-card", id: "syllabus-" + track.id },
      el("span", {
        class: "lm-track-meta",
        text: meta,
      }),
      el("h3", { text: track.title }),
      el("p", { text: track.description }),
      syllabusList(track, readPages, titles),
      next
        ? el(
            "div",
            { class: "lm-track-actions" },
            el("a", {
              class: "lm-btn",
              href: base() + "/" + next,
              text: done ? "Continue with lesson " + (done + 1) : "Start lesson 1",
            }),
          )
        : el("span", { class: "lm-track-meta", text: "Syllabus complete" }),
    )
  }

  function syllabusPreview(track, readPages, titles) {
    const details = el("details", { class: "lm-syllabus-preview" })
    details.append(
      el("summary", { text: "View " + (track.pages?.length ?? 0) + "-lesson syllabus" }),
      syllabusList(track, readPages, titles),
    )
    return details
  }

  function pathNavigator(state, index, slug) {
    const tracks = (index.tracks ?? []).filter(
      (track) =>
        state.tracks?.[track.id]?.status === "active" && (track.pages ?? []).includes(slug),
    )
    if (!tracks.length) return null
    const titles = pageTitles(index)
    const nav = el("nav", { class: "lm-panel lm-path-nav", "aria-label": "Learning path" })
    for (const track of tracks) {
      const at = track.pages.indexOf(slug)
      const previous = at > 0 ? track.pages[at - 1] : null
      const next = at + 1 < track.pages.length ? track.pages[at + 1] : null
      nav.appendChild(
        el(
          "div",
          { class: "lm-path-nav-track" },
          el(
            "div",
            { class: "lm-path-nav-title" },
            el("strong", { text: track.title }),
            el("span", {
              class: "lm-track-meta",
              text: "Lesson " + (at + 1) + " of " + track.pages.length,
            }),
          ),
          el(
            "div",
            ...
            previous
              ? el("a", {
                  class: "lm-btn",
                  href: base() + "/" + previous,
                  text: "← " + (titles.get(previous) ?? "Previous lesson"),
                })
              : null,
            el("a", { class: "lm-btn", href: base() + "/learn", text: "View syllabus" }),
            next
              ? el("a", {
                  class: "lm-btn",
                  href: base() + "/" + next,
                  text: (titles.get(next) ?? "Next lesson") + " →",
                })
              : el("a", { class: "lm-btn", href: base() + "/learn", text: "Finish path" }),
          ),
        ),
      )
    }
  ...
  function startTrack(state, track, status = "active") {
    state.settings.on = true
    state.settingsUpdatedAt = Date.now()
    setTrack(state, track.id, status)
    const now = Date.now()
    for (const slug of track.pages ?? []) setEnrolled(state, slug, true, now)
    save(state)
  }

  function renderLearn(state, index, root, repaint) {
    const tracks = index.tracks ?? []
    const progress = loadProgress()
    const readPages = readSet(progress)
    const titles = pageTitles(index)
    const enrolled = new Set(state.enrolled ?? [])
    const pool = (index.cards ?? []).filter((card) => enrolled.has(card.slug))
    const plan = session(state, pool, readPages)
    const active = tracks.filter((track) => state.tracks?.[track.id]?.status === "active")
    const current = active
      .map((track) => ({ track, slug: nextInTrack(track, readPages) }))
      .find((row) => row.slug)

    root.textContent = ""
    const today = el("div", { class: "lm-today" })
    today.appendChild(
      el(
        "div",
        { class: "lm-today-card" },
        el("strong", {
          text: plan.queue.length ? plan.queue.length + " cards ready" : "No reviews waiting",
        }),
        el("span", {
          class: "lm-meta",
          text:
            plan.reviews +
            " due · " +
            Math.min(plan.unseen, Math.max(0, state.settings.newPerDay - plan.introduced)) +
            " eligible new",
        }),
        el(
          "div",
          ...
          el("a", {
            class: "lm-btn",
            href: base() + "/review",
            text: plan.queue.length ? "Review now" : "Open review",
          }),
        ),
      ),
    )
    today.appendChild(
      el(
        ...
        el("span", {
          class: "lm-meta",
          text: current
            ? "Next: " + (titles.get(current.slug) ?? current.slug.split("/").pop())
            : active.length
              ? "You have read every page in your active path."
              : "Start with a bounded sequence instead of the whole field.",
        }),
        // Every state of this card gets something to press. Two of the three
        // used to get nothing: "Choose a path" told a reader on their first
        // visit to do exactly that and handed them no way to, and "Tracks
        // completed" congratulated them and then stopped. Beside a sibling card
        // that always carries a button, both read as unfinished rather than
        // deliberate — and on a phone the paths they name are below the fold.
        //
        // The destination for both is the path grid further down this page, so
        // it is an in-page anchor rather than a navigation. It is only offered
        // when there are paths to anchor to.
        el(
          "div",
          { class: "lm-track-actions" },
          current
            ...
            : tracks.length
              ? el("a", {
                  class: "lm-btn",
                  href: "#" + PATHS_ANCHOR,
                  text: active.length ? "Choose another path" : "Browse paths",
                })
              : null,
        ),
      ),
    ...
    if (active.length) {
      root.appendChild(
        el("h2", {
          class: "lm-field-heading",
          text: active.length === 1 ? "Your syllabus" : "Your active syllabi",
        }),
      )
      const syllabi = el("div", { class: "lm-syllabus-grid" })
      for (const track of active) syllabi.appendChild(syllabusCard(track, readPages, titles))
      root.appendChild(syllabi)
    }

    const requested = new URLSearchParams(window.location.search).get("field")
    const preferredFields = ["psychology-neuroscience-self-mastery", "machine-learning"]
    const fieldRank = (field) => {
      if (field === requested) return -1
      const preferred = preferredFields.indexOf(field)
      return preferred === -1 ? preferredFields.length : preferred
    }
    const fields = [...new Set(tracks.map((track) => track.section))].sort((a, b) => {
      const ranked = fieldRank(a) - fieldRank(b)
      return ranked || a.localeCompare(b)
    })
    let firstField = true
    for (const field of fields) {
      const heading = el("h2", { class: "lm-field-heading", text: readableField(field) })
      // What the today card's "Browse paths" and "Choose another path" point at.
      if (firstField) {
        heading.id = PATHS_ANCHOR
        firstField = false
      }
      root.appendChild(heading)
      const grid = el("div", { class: "lm-track-grid" })
      for (const track of tracks.filter((row) => row.section === field)) {
        const done = (track.pages ?? []).filter((slug) => readPages.has(slug)).length
        const total = track.pages?.length ?? 0
        const pct = total ? Math.round((done / total) * 100) : 0
        const status = state.tracks?.[track.id]?.status ?? "available"
        const next = nextInTrack(track, readPages)
        const phaseSteps = Array.isArray(track.phases)
          ? track.phases.flatMap((phase) => phase.steps ?? [])
          : []
        const guides = phaseSteps.filter((step) => step?.guide).length
        const practices = phaseSteps.filter((step) => step?.practice).length
        const trackMeta =
          Array.isArray(track.phases) && track.cards === 0
            ? [
                track.level,
                total + " lessons",
                guides ? guides + (guides === 1 ? " guide" : " guides") : null,
                practices
                  ? practices + (practices === 1 ? " practice set" : " practice sets")
                  : null,
              ]
                .filter(Boolean)
                .join(" · ")
            : track.level + " · " + total + " lessons · " + track.cards + " cards"
        const actions = el("div", { class: "lm-track-actions" })
        if (status === "active") {
          if (next)
            actions.appendChild(
              el("a", { class: "lm-btn", href: base() + "/" + next, text: "Continue" }),
            )
          actions.appendChild(
            el("button", {
              class: "lm-btn",
              ...
              text: "Pause",
              onclick: () => {
                setTrack(state, track.id, "paused")
                save(state)
                repaint()
              },
            }),
          ...
          actions.appendChild(
            el("button", {
              class: "lm-btn",
              type: "button",
              text: status === "paused" ? "Resume path" : "Start this path",
              onclick: () => {
          ...
          if (next)
            actions.appendChild(
              el("a", { class: "lm-btn", href: base() + "/" + next, text: "Preview first lesson" }),
            )
        }
        grid.appendChild(
          el(
            ...
            el(
              "div",
              { class: "lm-track-progress", title: done + " of " + total + " lessons read" },
              el("span", { style: "width:" + pct + "%" }),
            ),
            el("span", { class: "lm-track-meta", text: done + " of " + total + " lessons read" }),
            status === "active" ? null : syllabusPreview(track, readPages, titles),
            actions,
          ),
        )
      }
  ...
  function renderReview(state, index, root, onChange) {
    ...
    let plan = session(state, pool, readPages)
    ...
    const paint = () => {
      ...
      if (!state.settings.on) {
        // One panel, not two. This used to render the "off" notice and then
        // `startingPoints` beneath it, which is a second bordered block saying
        // "Nothing enrolled yet" and offering a second button — two competing
        // calls to action on the page a reader sees before they have done
        // anything. Worse, the one presented first was the weaker: `startTrack`
        // sets `settings.on` *and* enrols the path's pages, so choosing a path
        // does this button's job as a side effect, while pressing this button
        // alone only moves the reader to a second empty screen.
        //
        // So the path leads, unless the reader already has a scope — someone
        // who enrolled pages and later switched scheduling off is not starting
        // out, and should not be told to choose a path they may already have.
        const turnOn = el("button", {
          class: "lm-btn",
          type: "button",
          text: "Turn on scheduling",
          onclick: () => {
            state.settings.on = true
            state.settingsUpdatedAt = Date.now()
            save(state)
        ...
        const panel = el(
          "div",
          { class: "lm-panel" },
          el("strong", { text: "Nothing is scheduled yet." }),
          el("span", {
            text:
              index.cards.length +
              " cards are written across " +
              (index.pages ?? 0) +
              " pages. Scheduling is off, so nothing is due and no page holds you back. A path" +
              " enrols the pages it covers and turns scheduling on.",
          }),
        )
        if (pool.length) panel.append(turnOn, choosePath)
        else panel.append(choosePath, turnOn)
      ...
      const summary = el(
        "div",
        { class: "lm-panel" },
        el("span", { text: pool.length + " cards enrolled" }),
        ...
        plan.unread
          ? el("span", {
              class: "lm-due",
              text: plan.unread + " new cards wait until their pages are read",
            })
          : null,
      )
      if (!pool.length) {
        // The summary is deliberately not appended here. With nothing enrolled
        // every one of its figures is zero, and it used to sit above two more
        // lines that said the same thing in words: `0 cards enrolled · 0 left
        // today · 0 due · 0 eligible new`, then "Nothing enrolled yet.", then
        // "Nothing enrolled yet" again as the heading below it. Three ways of
        // reporting the same nothing, in a column. The section hub band was
        // changed for the same reason — a measure with nothing to measure is
        // not a reading, it just looks like a broken feature.
        root.appendChild(
          el("p", {
            class: "lm-empty",
            text:
        ...
        root.appendChild(startingPoints(index, false))
        root.appendChild(exportBar(state))
        return
      }
      ...
      if (!plan.queue.length) {
        ...
        root.appendChild(exportBar(state))
        root.appendChild(enrolledList(state, index, pool, paint))
        ...
        if (plan.soon) {
          wake = window.setTimeout(
            () => {
              plan = session(state, pool, readPages)
              paint()
            },
      ...
      bar.appendChild(
        el("button", { class: "lm-btn", type: "button", text: "Show answer", onclick: reveal }),
      )
  ...
  function finished(state, plan, sitting) {
    ...
    if (!sitting.done) {
      if (!waiting && !plan.held && plan.unread) {
        wrap.appendChild(
          el("p", {
            class: "lm-empty",
            text: "No reviews are due. Continue a learning path to encounter the next material.",
          }),
        )
        wrap.appendChild(
          el("a", { class: "lm-btn", href: base() + "/learn", text: "Continue learning" }),
        )
        return wrap
      }
      wrap.appendChild(
          ...
          text:
            ...
            (plan.held
              ? "Nothing left for today. " +
                plan.held +
                " cards are waiting on tomorrow's allowance."
    ...
    wrap.appendChild(
      el("p", { class: "lm-figure", text: sitting.done + " cards · " + pct + "% kept" }),
    )
    wrap.appendChild(
      el("p", {
        class: "lm-meta",
        text:
          ...
          (waiting ||
            (plan.held
              ? plan.held +
                " more are held for tomorrow — the cap is what keeps a section learnable."
  ...
  function exportBar(state) {
      ...
      el("button", {
        ...
        onclick: () => {
          ...
          const a = el("a", {
            ...
            download: "symposium-progress.json",
          })
      ...
      try {
        ...
        if (incoming.progress) {
          window.localStorage.setItem(
            PROGRESS_KEY,
            JSON.stringify(mergeProgress(loadProgress(), incoming.progress)),
          )
        }
        save(state)
    ...
    bar.appendChild(
      el("span", {
        class: "lm-meta",
        text: "Export keeps a service-independent backup of both cards and the reading map.",
      }),
    )
    return bar
  }

  /** The queue is empty on every first visit, so point to authored paths. */
  /**
   * The way in, for a reader with nothing enrolled.
   *
   * `heading` is off when the caller has already said "Nothing enrolled yet" in
   * its own words — repeating it two lines later was the whole defect this
   * argument exists to prevent.
   */
  function startingPoints(index, heading = true) {
    const wrap = el("div", { class: "lm-enrolled" })
    if (!index.cards?.length) return wrap
    if (heading) wrap.appendChild(el("strong", { text: "Nothing enrolled yet" }))
    wrap.appendChild(
      el("p", {
        class: "lm-meta",
        text: "Choose an ordered learning path first; cards enter review only after you read their pages.",
      }),
    )
    wrap.appendChild(
      el("a", { class: "lm-btn", href: base() + "/learn", text: "Choose a learning path" }),
    )
    return wrap
  }

  function enrolledList(state, index, pool, repaint) {
    const wrap = el("div", { class: "lm-enrolled" })
    if (!state.enrolled.length) return wrap
    const activeTracks = (index.tracks ?? []).filter((track) => state.tracks?.[track.id]?.status)
    const covered = new Set(activeTracks.flatMap((track) => track.pages ?? []))
    wrap.appendChild(el("strong", { text: "Learning scope" }))
    const ul = el("ul")
    for (const track of activeTracks) {
      const status = state.tracks?.[track.id]?.status ?? "paused"
      ul.appendChild(
        el(
          "li",
          {},
          ...
          el("span", {
            class: "lm-meta",
            text: " — " + status + ", " + track.pages.length + " lessons",
          }),
        ),
      )
    }
    for (const slug of state.enrolled.filter((name) => !covered.has(name))) {
    ...
    wrap.appendChild(ul)
    wrap.appendChild(
      el("a", { class: "lm-btn", href: base() + "/learn", text: "Manage learning paths" }),
    )
    return wrap
  }
  ...
  function teardown() {
    document
      .querySelectorAll(
        ".lm-panel, .lm-grade, .lm-cardbar, .lm-lock, .lm-pass, #lm-review, #lm-learn",
      )
      .forEach((n) => n.remove())
  ...
  function page(state, index, rerun) {
    const center =
      document.querySelector(".center") ?? document.querySelector("article")?.parentElement
    if (!center) return

    if ((document.body.dataset.slug ?? "") === "learn") {
      const root = el("div", { id: "lm-learn" })
      center.appendChild(root)
      const repaint = () => renderLearn(state, index, root, repaint)
      repaint()
      return
    }
    ...
    const gated =
      entries.length && state.settings.hard && article && wireHard(state, entries, article)
    ...
    const pathNav = pathNavigator(state, index, document.body.dataset.slug ?? "")
    if (pathNav) center.appendChild(pathNav)
  }

  document.addEventListener("nav", setup)
  document.addEventListener("render", setup)
  window.addEventListener("symposium:cloud-merged", setup)
})();